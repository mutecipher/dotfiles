...
#+end_src
** FRINGE
Wider left fringe gives Flymake indicators breathing room and reduces the
cramped-edge feeling nano-emacs avoids.
...
#+end_src
** BLOG
Creates a new blog post file in =~/Documents/Blog/= with pre-filled Org metadata.
Invoke with =C-c b n=.
...
#+end_src
** MODE-LINE
Custom mode line with Nerd Font icon for the current buffer's file type or major mode.
  ...
  (setq-default mode-line-format
    '("%e"
      ...
      (:eval (cond
	      ...
	      (t                   "  ")))
      (:eval (concat "("
		     (propertize (car (split-string (string-trim (format-mode-line mode-name)) "/"))
				 'face 'bold)
		     ")"))
      "  "
      (:eval (when vc-mode
	       (let ((branch (string-trim-left vc-mode "[ Git@:-]+")))
		 (concat (propertize "\uf126" 'face 'mutecipher-icon-red)
			 " " branch))))
      "  "
      (:eval (mutecipher/buffer-icon))
      " "
      ...
      (:eval (when which-function-mode
		 ...
		 (concat "  "
			 (propertize "\uf105" 'face 'mutecipher-icon-dim)
			 " "
