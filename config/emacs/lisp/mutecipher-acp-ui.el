    ...
    (cond
     ...
     (t
      (let ((wrapper (ewoc-data node)))
        (setf (mutecipher-acp--node-collapsed-p wrapper)
...
composer region."
  ...
  (let* ((wrappers (ewoc-collect mutecipher-acp--ewoc
         ...
         (any-expanded (cl-some (lambda (d) (not (mutecipher-acp--node-collapsed-p d)))
                                wrappers))
         (new-collapsed (and any-expanded t)))
    (if (null wrappers)
        (message "ACP: no tool calls to toggle")
      (dolist (wrapper wrappers)
        (setf (mutecipher-acp--node-collapsed-p wrapper) new-collapsed))
      (mutecipher-acp--with-sticky-window-start (current-buffer)
        (let ((inhibit-read-only t))
